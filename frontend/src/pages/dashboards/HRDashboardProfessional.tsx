...
const ProfessionalHRDashboard: React.FC = () => {
  ...
  // Fetch HR dashboard data with error handling and retry logic
  const fetchHRDashboardData = useCallback(async (showLoading = false, retryCount = 0) => {
    try {
      ...
      // Calculate notifications from pending actions
      ...
      console.log('HR Dashboard data loaded successfully:', {
        employees: response.data.organizationStats?.totalEmployees,
        pendingLeaves: response.data.leaveStats?.totalPendingLeaves,
      ...
      if (retryCount < 3 && (error.code === 'NETWORK_ERROR' || error.response?.status >= 500)) {
        ...
        setTimeout(() => fetchHRDashboardData(showLoading, retryCount + 1), 2000 * (retryCount + 1));
        return;
      }
      ...
      <Paper elevation={0} sx={{ p: 3, borderRadius: 3 }}>
        ...
        <Grid container spacing={2}>
          {[
            ...
            { icon: PolicyIcon, label: 'Policies & Compliance', path: '/policies', color: '#ef6c00' },
